...
import { Product } from "@/lib/fake-data";

type ProductProps = {
  fakeproduct: Product;
};

export default function CatalogueCard({ fakeproduct }: ProductProps) {
  return (
    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.8 }}>
      <Card className="w-full h-[550px]  shadow-xl rounded-2xl overflow-hidden border-none">
        ...
        <CardContent className="px-7 py-5">
          <p className="text-lg font-bold text-black mb-5">
            {/* Double Bombed Chair & Wood Style */}
            {fakeproduct.name}
          </p>
          <div className="flex items-center justify-between space-x-2">
            <div className="flex space-x-3">
              {/* <span className="text-sm line-through text-[#8D8D8D]">$250.00</span> */}
              <span className="text-sm line-through text-[#8D8D8D]">
                $ {fakeproduct.price}
              </span>
              {/* <span className="text-sm font-bold text-black">$200.00</span> */}
              <span className="text-sm font-bold text-black">
                $ {fakeproduct.price - 20}
              </span>
            </div>
            <div>
